    ...
    public static void main(String[] args) throws IOException {
        ...
        if (in.available() > 0) {
            ...
            while ((entry = arc.next()) != null) {
                if (entry.getHash() == Utils.getArchiveHash("npc.dat")) {
                    data = ByteBuffer.wrap(IOUtils.toByteArray(arc));
                } else if (entry.getHash() == Utils.getArchiveHash("npc.idx")) {
        ...
        NpcDecoder decoder = new NpcDecoder(data, indexDecoder.decode());
